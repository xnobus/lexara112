...
bool MSDFFont::CreateAtlasPage() {
    ...
    if (!D3D::CreateTexture(&page->texture, {
        ...
        Log("[MSDF] CreateAtlasPage: CreateTexture %ux%u ODMOWIL (stron dotad=%u, max=%u)",
            ...
            (unsigned)m_atlasPages.size(), (unsigned)MSDF::MAX_ATLAS_PAGES);
        return false;
    }
    Log("[MSDF] CreateAtlasPage: strona %u utworzona (tekstura=%p)",
        (unsigned)m_atlasPages.size(), (void*)page->texture);
...
bool MSDFFont::UploadGlyphToAtlas(GlyphMetrics& metrics, uint32_t codepoint) {
    ...
    if (pageIndex == -1) {
        if (m_atlasPages.size() >= MSDF::MAX_ATLAS_PAGES) {
            ...
            if (!CreateAtlasPage()) { ++gNullUploadGen; return false; }
